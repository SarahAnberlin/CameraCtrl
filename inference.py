...
from my_dataset.realestate10k import RealEstate10KPose
import argparse
import json
import os
...
def get_pipeline(
    ...
    unet = UNet3DConditionModelPoseCond.from_pretrained_2d(
        pretrained_model_path="/hpc2hdd/home/hongfeizhang/hongfei_workspace/CameraCtrl/fused_lora",
...
def main(args):
    ...
    for idx, batch in tqdm(enumerate(dataloader)):
        ...
        #         print(f"rank {rank} / {torch.cuda.device_count()}, {k}: {v.shape}")
        #     elif isinstance(v, list):
        #         print(f"rank {rank} / {torch.cuda.device_count()}, {k}: {len(v)}")
        #     else:
        #         print(f"rank {rank} / {torch.cuda.device_count()}, {k}: {v}")
        ...
        print(f"input data prompt: {input_data['prompt']}")
        generator.manual_seed(42)
        sample = pipeline(
            prompt=input_data["prompt"],
            negative_prompt=None,
            pose_embedding=input_data["plucker_embedding"],
        ...
        save_name = "_".join(input_data["prompt"][0].split())
