...
class ScenesDatasetAltered(Dataset):
    ...
    def __init__(
        ...
        self.max_frame = num_frame
        self.min_frame = num_frame
        self.debug = debug
    ...
    def __getitem__(
        ...
        print(f"Getting item {idx} with meta path: {meta_path}")
        try:
            ...
            if self.split == 'train':
                ...
                # 验证阶段默认固定长度和步长
                sample_step = self.sample_step # TODO
...
if __name__ == "__main__":
    ...
    random.seed(seed)
    parser_type = 'colmap'
    resize_size = [480, 320]  # W H
    sample_step = 1
    frame_length = 41
    dataset = ScenesDatasetAltered(    split="test", ratio=0.03, patch_size=[480, 320], resize_size=resize_size, sample_step=sample_step, num_frame=frame_length,)
